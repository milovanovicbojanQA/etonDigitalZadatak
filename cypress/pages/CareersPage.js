...
//     CareersPage.loadMorePositions(); // Učitaj sve pozicije
//     cy.wait(2000);

//     cy.get(Selectors.jobTitle).then(($titles) => {
//       const jobTitles = $titles.map((i, el) => Cypress.$(el).text()).get();
//       cy.writeFile('serbia-job-titles.txt', jobTitles.join('\n')); // Snimi u fajl
//     });
//   }
...
static getAllJobTitles() {
    const jobTitlesArray = []; // Niz za skladištenje naziva poslova
        ...
        if ($button.is(':visible')) {
          ...
          cy.get('body > main > div.c-section.c-section--careers > div') // Koristite vaš selektor
            ...
            .then(($titles) => {
              ...
              cy.writeFile('serbia-job-titles.txt', jobTitlesArray.join('\n')); // Zapiši sve nazive poslova u datoteku
            });
        }
      });
    };
